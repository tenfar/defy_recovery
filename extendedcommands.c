...
void show_nandroid_restore_menu()
{
    ...
    char* file = choose_file_menu("/sdcard/goapkrev/backup/", NULL, headers);
    if (file == NULL)
        return;
...
{
    struct stat info;
    if (0 == stat("/sdcard/goapkrev/.no_confirm", &info))
        return 1;
...
int run_and_remove_extendedcommand()
{
    ...
    remove("/sdcard/goapkrev/.recoverycheckpoint");
    if (i == 0) {
...
void show_nandroid_advanced_restore_menu()
{
    ...
    char* file = choose_file_menu("/sdcard/goapkrev/backup/", NULL, advancedheaders);
    if (file == NULL)
        return;
        ...
        case 0:
            {
                ...
                if (tmp == NULL)
                {
                    ...
                    sprintf(backup_path, "/sdcard/goapkrev/backup/%d", tp.tv_sec);
                }
                else
                {
                    strftime(backup_path, sizeof(backup_path), "/sdcard/goapkrev/backup/%F.%H.%M.%S", tmp);
                }
...
void handle_failure(int ret)
{
    ...
    if (0 != ensure_root_path_mounted("SDCARD:"))
        return;
    mkdir("/sdcard/goapkrev", S_IRWXU);
    __system("cp /tmp/recovery.log /sdcard/goapkrev/recovery.log");
    ui_print("/tmp/recovery.log was copied to /sdcard/goapkrev/recovery.log. Please open ROM Manager to report the issue.\n");
}
